require_relative "../bluegill"

describe Bluegill do
  let(:mascot) { Bluegill.new() }
  ...
  describe "base stats" do
    it "has a base hit point value" do
      expect(mascot.base_hit_point_value).to eq 45
    end

    it "has a base attack strength" do
      expect(mascot.base_attack_strength).to eq 67
    end

    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 60
    end
  end
  ...
  describe "attacking and defending" do
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:water_pulse, :bubble_beam, :aqua_ring, :soak]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:aqua_ring)).to be true
      end
