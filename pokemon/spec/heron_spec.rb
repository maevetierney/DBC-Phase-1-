require_relative "../heron"

describe Heron do
  let(:mascot) { Heron.new() }
  ...
  describe "base stats" do
    it "has a base hit point value" do
      expect(mascot.base_hit_point_value).to eq 75
    end

    it "has a base attack strength" do
      expect(mascot.base_attack_strength).to eq 87
    end

    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 63
    end
  end
  ...
  describe "attacking and defending" do
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:wing_attack, :air_slash, :air_cutter, :gust]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:air_cutter)).to be true
      end
