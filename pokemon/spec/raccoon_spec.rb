...
describe Raccoon do
  ...
  describe "base stats" do
    ...
    it "has a base attack strength" do
      expect(mascot.base_attack_strength).to eq 75
    end

    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 85
    end
  end
  ...
  describe "attacking and defending" do
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:sand_attack, :earthquake, :scratch, :fury_swipes]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:scratch)).to be true
      end
